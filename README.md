...
## Confidentialite et publication GitHub

Avant chaque push:

1. Verifier que ces fichiers ne sont pas suivis par git:
...
2. Si un fichier sensible est deja tracke, le retirer de l'index:

```powershell
git rm --cached .env resume.json startups.json generated_emails.json email_tracking.json
```
...
```powershell
git add .
git commit -m "Prepare project for public GitHub usage"
git push
```
...
Important:
- Ne jamais mettre de cle API, email personnel, telephone, lien CV prive dans les fichiers versionnes.
- Garder les vraies donnees uniquement dans les fichiers locaux ignores.

## Depannage

- Erreur `GEMINI_MODEL is not set`: ajouter `GEMINI_MODEL` dans `.env`.
- Erreur SMTP authentication: verifier `GMAIL_USER`, `GMAIL_APP_PASSWORD` (mot de passe d'application Gmail).
- Aucune offre scrapee: verifier la connectivite reseau et la disponibilite du site source.
